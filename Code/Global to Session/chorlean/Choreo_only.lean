...
inductive Choreo {δ:Type} (ep:δ)  [LocSig δ]: (p: δ -> Prop := fun _ => true) -> [Decidable (p ep)] ->  Type -> Type 1  where
| Cond {μ} {α} {decider:δ} [DecidableEq δ] [FinEnum δ] [Serialize μ]: GVal decider ep μ -> (μ -> Choreo ep p α) -> Choreo ep p α
| Only   (q: δ -> Prop) {α} :  Choreo ep (fun x => p x ∧ q x) α ->  Choreo ep p α -> Choreo ep p α
| Do {α β}:  ChorEff ep β -> (β -> Choreo ep p α) -> Choreo ep p α
| Return {α}: α -> Choreo ep p α (δ:=δ)

def Choreo.bind {ep:δ} [LocSig δ]{α β: Type} {p: δ -> Prop} :
  Choreo ep p α  → (α → Choreo ep p β) → Choreo ep p β
...
| Choreo.Only q sub_chor chor, next' =>
  if h:(q ep) then
    bind (chor) next'
  else
    bind (chor) next'
...
instance {δ:Type} [DecidableEq δ]  [LocSig δ]{ep:δ} (p: δ -> Prop): Monad (Choreo ep p (δ:=δ)) where
