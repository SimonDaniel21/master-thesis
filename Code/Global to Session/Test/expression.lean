...
inductive EXPRESSION where
  | INPUT_VAR (v: Variable) : EXPRESSION
...
def eval_expression (var_map: List (Variable × Nat)): EXPRESSION -> EXPRESSION_RESULT Nat
  | EXPRESSION.VAR n =>
  ...
  | EXPRESSION.DIVIDE e_a e_b =>
    do
    let v_a <- eval_expression var_map e_a
    let v_b <- eval_expression var_map e_b
...
#eval (e_1)
#eval (eval_expression vars e_1 == EXPRESSION_RESULT.some 74)

#eval (e_2_nan)
#eval (eval_expression vars e_2_nan == EXPRESSION_RESULT.DIV_BY_ZERO)

#eval (e_3_unknown)
#eval (eval_expression vars e_3_unknown == EXPRESSION_RESULT.UNKNOWN_VAR "v_unknown")

#eval (e_4_var)
#eval (eval_expression vars e_4_var == EXPRESSION_RESULT.some 1)
