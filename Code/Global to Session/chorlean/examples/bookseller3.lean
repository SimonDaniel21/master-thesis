...
def pay_rest (borrower lender endpoint: String)
  : negT2 (l1:=borrower) (endpoint:= endpoint) := fun budget price => do
...
def book_seller (endpoint: String) (negotiate: negT2  (l1:="buyer") (endpoint:=endpoint)) : Choreo endpoint (Option (GVal String "buyer" endpoint)):= do
  ...
  let temnop <- send_recv_comp endpoint "buyer" "seller" (do
    IO.println "enter a book title:"
    return <- IO.getLine)
