...
class Printer (m: Type u -> Type v) where
  print: String -> m Unit

instance: Printer IO where
  print := IO.println

def t_prog : (Printer IO) Nat := do
  return
...
instance {loc ep: δ} [ToString t]: ToString (GVal loc ep t) where
  toString x :=
    if h:(loc = ep) then
      let val := x.unwrap h
      toString val
...
class Network (δ:Type) [DecidableEq δ] (ep: δ) where
  com  {t:Type} [Serialize t]: (s: δ) -> GVal  s ep t-> (r: δ) -> IO (GVal r ep t)
...
abbrev LVal (owner: δ) (α) := fun (ep:δ) => GVal owner ep α

inductive ChorEff' {δ:Type} (ep:δ):  IO  -> Type 1  where
| Send_recv [ser:Serialize a]: {sender:δ} -> GVal sender ep a -> (receiver:δ) -> ChorEff' ep (LVal receiver α)
| Local : (loc:δ) -> (  IO a) -> ChorEff' ep (LVal loc a)
universe u v

inductive Choreo' (ep:δ): (δ -> Type) -> Type 1  where
| Cond [Serialize a] {decider:δ}: GVal decider ep a -> (a -> Choreo' ep b) -> Choreo' ep b
| Cond2 {decider:δ}: GVal decider ep Bool -> (Choreo' ep a) -> (Choreo' ep a) -> Choreo' ep a
| Do :  ChorEff' ep b -> ((b ep) -> Choreo' ep a) -> Choreo' ep a
| Return {a:Type}: a -> Choreo' ep (fun _ep => a)

def Choreo'.bind {α β}:
  Choreo' ep α → ((α ep) → Choreo' ep β) → Choreo' ep β
...
def Choreo'.pure (v: α):  Choreo' ep (fun _ => α) :=
  Choreo'.Return v

def not_ep {δ:Type} := (δ -> Type) -> Type

instance: Monad (Choreo ep not_ep) where
  bind := Choreo.bind
  pure x := fun _ep => Choreo'.Return x

def dist_prog: Choreo' (LVal alice Nat) :=
  return sorry
...
def locally
  (loc: δ) (comp: IO b): Choreo' ep (LVal loc b) (δ:=δ) :=
  toChoreo (ChorEff'.Local loc comp) (a:=LVal loc b)
...
def Choreo.bind {α β} (x : Choreo δ α) (f : α → Choreo δ β) : Choreo δ β :=
  fun r => do
    -- TODO besser beschreiben wenn ich wacher bin
    let temp := x r
    let temp2 <- temp
    let temp3 := f temp2
    let temp4 := temp3 r
...
def LVal (owner: δ) (α:Type): Choreo' δ Type :=
  fun ep => Choreo'.Return (GVal owner ep α)

#check LVal (δ:= Example_Location) alice Nat
#check Choreo Example_Location (LVal (δ:= Example_Location) alice Nat)


def locally (owner: δ): Choreo' δ ((ep:δ) -> GVal owner ep α):=sorry

def send_recv {a:Type} [Serialize a] {sender:δ}  (receiver: δ):
  Choreo' δ ((ep:δ) -> GVal sender ep a -> GVal receiver ep a) := sorry
...
def dist_prog: Choreo Example_Location Nat := do
  let t <- LVal alice Nat
  -- let t2 := locally alice (α := Nat)
  -- let t3 <- t2
