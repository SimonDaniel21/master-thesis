...
class Serialize (μ: Type) extends ToString μ where
  ...
  type_name: String
  pretty: μ -> String := fun x => s!"{x}: {type_name}"
...
def Socket.send_val2 (sock: Socket) (msg: μ): IO Unit := do
  ...
  if payload.size >= UInt32.size then
      IO.println s!"cannot send payloads with more than UInt32 bytes size"
  let size_info: ByteArray := payload.size.toUInt32.to_bytes
